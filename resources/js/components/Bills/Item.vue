<template>
  <tr>
    <td>
      ...
      <el-text class="mx-1 text-danger" v-if="isNewItem"
        >This will create a new item</el-text
      >
    </td>
    <td>
      <el-input
    ...
    <td v-if="operation !== 'view'">
      <el-button
        type="danger"
        @click="getDeletedItemsId(index, item.id)"
...
export default {
  ...
  methods: {
    ...
    calculateTotal() {
      this.item.total =
        parseFloat(this.item.rate) * parseFloat(this.item.quantity);
      let summation = this.bill.items.reduce(
  ...
  computed: {
    formattedRate: {
      get() {
        return this.item.rate.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 4,
        }); // Apply precision formatting when retrieving the value
      },
      set(value) {
        this.item.rate = value; // Convert the input value back to a number
      },
    },
    formattedTotal() {
      return this.item.total
        ? this.item.total.toLocaleString("en-US", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 4,
          })
        : "0.00"; // Apply precision formatting
    },
  },
};
</script>
