          ...
          <table class="table table-borderless">
            <thead>
              <tr>
                <th
                  :style="
                    operation === 'view' ? { width: '25%' } : { width: '15%' }
                  "
                >
...
export default {
  ...
  async created() {
    ...
    if (this.bill.id) {
      axios.get(`/api/bills/edit/` + this.bill.id).then((res) => {
        console.log("res:", res);
  ...
  methods: {
    ...
    async createBill() {
      ...
      try {
        await axios.post(`/api/bills`, this.bill).then((res) => {
    ...
    async updateBill() {
      ...
      try {
        await axios
          .post(`/api/bills/` + this.bill.id, this.bill)
          .then((res) => {
            console.log("res:", res, this.$router);
            this.$router.push("/" + this.$route.params.slug + "/bills");
          });
